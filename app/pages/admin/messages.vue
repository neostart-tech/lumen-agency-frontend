      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div class="flex items-center gap-3">
            <div
              class="stats-pill bg-primary/10 text-primary px-4 py-2 rounded-xl border border-primary/20 flex items-center gap-2">
        ...
        <div class="flex flex-wrap items-center gap-4 px-6 py-4 border-b border-gray-50 bg-[#fafafa]/50">
          <div class="relative w-full sm:w-64">
            ...
            <input v-model="search" type="text" placeholder="Nom, email, objet..."
              class="pl-10 pr-4 py-2.5 w-full rounded-xl text-sm border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-medium" />
          </div>

          <select v-model="filterStatus"
            class="px-4 py-2.5 rounded-xl text-sm font-semibold border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all cursor-pointer">
          ...
          <div class="relative" ref="dropdownRef">
            <button @click="isDropdownOpen = !isDropdownOpen"
              class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold bg-primary/10 text-primary hover:bg-primary/20 transition-all">
            ...
            <transition name="dropdown">
              <div v-if="isDropdownOpen"
                class="absolute left-0 mt-2 w-48 bg-white rounded-xl shadow-xl z-20 border border-gray-100">
                <div class="py-1 max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-200">
                  <label v-for="col in allColumns" :key="col.field"
                    ...
                    <input type="checkbox" v-model="col.visible" class="rounded accent-primary" />
          ...
          <button @click="resetFilters"
            class="text-xs font-bold text-gray-400 hover:text-primary transition-colors flex items-center gap-1">
        ...
        <div class="p-6">
          <vue3-datatable :rows="filteredRows" :columns="visibleColumns" :search="search" :sortable="true"
            ...
            :pageSize="10" :totalRows="messages?.length || 0" :noDataContent="noDataMessage"
            ...
            <template #expediteur="data">
              <div class="flex items-center gap-3">
                <div class="h-8 w-8 rounded-lg flex items-center justify-center font-bold text-xs"
                  :style="{ background: data.value.is_read ? '#f1f5f9' : 'rgba(var(--color-primary-rgb), 0.1)', color: data.value.is_read ? '#64748b' : 'var(--color-primary)' }">
                  {{ data.value.expediteur.charAt(0) }}
                </div>
                <div class="flex flex-col">
                  <span class="text-sm font-bold" :class="data.value.is_read ? 'text-gray-600' : 'text-black'">{{
                    data.value.expediteur }}</span>
                  <span v-if="!data.value.is_read"
                    class="text-[10px] text-primary font-extrabold uppercase tracking-tighter">Nouveau</span>
                </div>
              </div>
            </template>
            ...
            <template #type="data">
              <span class="text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-md"
                :class="data.value.type === 'devis' ? 'bg-amber-50 text-amber-600' : 'bg-secondary/10 text-secondary'">
                {{ data.value.type || '—' }}
              </span>
            </template>

            <template #date_prevue="data">
              <span class="text-sm font-medium text-gray-600">
                {{ data.value.date_prevue || '—' }}
              </span>
            </template>

            <template #budget="data">
              <span class="text-sm font-semibold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md"
                v-if="data.value.budget">
                {{ data.value.budget }}
              </span>
        ...
        <div
          ...
          <div class="flex items-center justify-between px-8 py-6 border-b border-gray-50">
            <div class="flex items-center gap-4">
              <div
                class="h-12 w-12 rounded-2xl bg-primary flex items-center justify-center text-white shadow-lg shadow-primary/20">
              ...
              <div>
                ...
                <p class="text-xs font-semibold uppercase tracking-widest mt-0.5"
                  :class="selectedMessage.type === 'devis' ? 'text-amber-500' : 'text-secondary'">
                  Demande de {{ selectedMessage.type || 'Contact' }}
                </p>
              </div>
            </div>
          ...
          <div class="px-8 py-6 space-y-6 max-h-[60vh] overflow-y-auto">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="col-span-2 bg-primary/5 rounded-2xl p-4 border border-primary/10">
                <p class="text-[10px] font-bold text-primary/40 uppercase tracking-widest mb-1">Email</p>
                <p class="text-sm font-bold text-primary">{{ selectedMessage.email }}</p>
              </div>
              <div class="bg-gray-50 rounded-2xl p-4">
                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Date Prévue</p>
                <p class="text-sm font-bold text-black">{{ selectedMessage.date_prevue || '—' }}</p>
              </div>
              <div class="bg-gray-50 rounded-2xl p-4 text-right">
                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Budget</p>
                <div class="mt-1">
                  <span v-if="selectedMessage.budget"
                    class="text-sm font-semibold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md">{{
                      selectedMessage.budget }}</span>
                  <span v-else class="text-sm font-bold text-black">—</span>
                </div>
              </div>
            </div>
            ...
            <div class="space-y-2">
              ...
              <div class="bg-gray-50 rounded-2xl p-4">
                <p class="text-sm font-bold text-primary leading-relaxed">{{ selectedMessage.objet }}</p>
              </div>
            </div>
          ...
          <div class="px-8 py-5 border-t border-gray-50 bg-gray-50/50 flex justify-end gap-3">
            <button @click="sendReply(selectedMessage)" :disabled="isSendingReply"
              class="inline-flex items-center gap-2 px-6 py-3 bg-primary hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-bold rounded-xl transition-all shadow-md active:scale-95">
...
const allColumns = ref([
  ...
  { field: 'type', title: 'Type', visible: true, sort: true, width: '120px' },
  { field: 'date_prevue', title: 'Date Prévue', visible: true, sort: true, width: '130px' },
  { field: 'budget', title: 'Budget', visible: true, sort: true, width: '110px' },
