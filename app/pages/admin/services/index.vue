<template>
  <div class="services-page min-h-screen bg-[#f8fafc] p-6 space-y-6">
    <!-- Breadcrumb -->
    <AdminBreadcrumb :items="[{ label: 'Dashboard', link: '/admin' }, { label: 'Services' }]" />

    <!-- Loader circulaire autour du logo -->
    <AdminLoader v-if="serviceStore.loading && services.length === 0" :visible="true" inline />

    <div v-if="!serviceStore.loading || services.length > 0" class="content-wrapper">
      <!-- HEADER CARD -->
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 class="text-xl  text-black">Gestion des Services d'Activité</h2>
            <p class="text-xs text-gray-400 mt-1 uppercase tracking-wider ">
              Consultez, ajoutez ou modifiez les services d'expertise
            </p>
          </div>
          <div class="flex items-center gap-3">
            <div
              ...
              <span class="text-xs ">{{ services.length }} Services enregistrés</span>
            </div>
            <NuxtLink to="/admin/services/ajouter" class="btn-add">
        ...
        <div class="flex flex-wrap items-center gap-4 px-6 py-4 border-b border-gray-50 bg-[#fafafa]/50">
          <div class="relative w-full sm:w-64">
            ...
            <input v-model="searchQuery" type="text" placeholder="Rechercher par titre, description..."
              class="pl-10 pr-4 py-2.5 w-full rounded-xl text-sm border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all " />
          </div>

          <!-- COLUMNS TOGGLE -->
          <div class="relative" ref="dropdownRef">
            <button @click="isDropdownOpen = !isDropdownOpen"
              class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm  bg-primary/10 text-primary hover:bg-primary/20 transition-all">
            ...
            <transition name="dropdown">
              <div v-if="isDropdownOpen"
                ...
                <div class="py-1 max-h-32 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-200">
        ...
        <!-- TABLE -->
        <div class="p-6">
          <vue3-datatable :rows="services" :columns="visibleColumns" :search="searchQuery" :sortable="true"
            :sortColumn="'titre'" :sortOrder="'asc'" :loading="serviceStore.loading"
            skin="bh-table-hover bh-table-compact" class="custom-datatable" :pageSize="10" :totalRows="services.length"
            ...
            <template #image="data">
              <div
                ...
                :style="data.value.image ? { backgroundImage: `url(${getImageUrl(data.value.image)})` } : {}">
            ...
            <template #actions="data">
              <div class="flex items-center justify-center gap-2">
                <NuxtLink :to="'/admin/services/' + data.value.id" class="action-btn edit" title="Modifier">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
const noDataMessage = computed(() => {
  if (searchQuery.value) return `Aucun service trouvé pour "${searchQuery.value}"`;
...
const confirmDelete = async (service: any) => {
  const result = await Swal.fire({
    ...
    text: `Cela supprimera également toutes les références liées à "${service.titre}".`,
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    try {
      await serviceStore.destroy(service.id);
      Swal.fire({
        icon: 'success',
        title: 'Supprimé !',
