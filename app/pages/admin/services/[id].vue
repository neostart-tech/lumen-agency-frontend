<template>
  <div class="services-modifier-page">
    <AdminBreadcrumb :items="[{ label: 'Services', link: '/admin/services' }, { label: 'Modifier un service' }]" />
    ...
    <div v-if="loadingService" class="bg-white rounded-2xl h-96 animate-pulse border border-gray-200"></div>

    <div v-else class="content-card">
      <form @submit.prevent="updateService" class="admin-form">
        <div class="form-grid">
          <!-- Colonne Principale -->
          <div class="form-main">
            <div class="form-group">
              <label>Titre du service</label>
              <input type="text" v-model="form.titre" placeholder="Ex: Evenementiel..." required />
            </div>

            <div class="form-group">
              <label>Description</label>
              <textarea v-model="form.description" rows="6"
                placeholder="Spécifiez les activités, l'approche et la valeur ajoutée de ce service..."></textarea>
            </div>
          </div>
        ...
        <div class="form-actions border-top">
          <NuxtLink to="/admin/services" class="btn-cancel">Annuler</NuxtLink>
          <button type="submit" class="btn-save btn-save-edit" :disabled="serviceStore.loading">
            <component :is="IconSave" class="icon-sm" />
            {{ serviceStore.loading ? 'Mise à jour...' : 'Mettre à jour le service' }}
          </button>
        </div>
      </form>
    </div>
...
onMounted(async () => {
  try {
    const data = await serviceStore.show(sectorId)
    form.value = {
      titre: data.titre,
    ...
    Swal.fire('Erreur', 'Impossible de charger les données du service.', 'error')
...
const updateService = async () => {
  ...
  try {
    await serviceStore.update(sectorId, formData)
    Swal.fire({
      icon: 'success',
    ...
    Swal.fire('Erreur', 'Impossible de modifier le service.', 'error')
  }
}
</script>

<style scoped>
.services-modifier-page {
  display: flex;
